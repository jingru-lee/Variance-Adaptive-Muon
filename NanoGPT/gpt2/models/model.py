...
from optimizers.muon_nsr import Muon_NSR
from optimizers.muon_vs import Muon_VS
...
class GPT(nn.Module):
    ...
    def configure_optimizers(self, optimizer_name, weight_decay, learning_rate, betas, device_type):
        """
        Configure and return optimizers for training.
        
        This method implements a hybrid optimization strategy where different parameter groups
        are handled by different optimizers:
        - For 'adam': All parameters use AdamW
        - For 'muon', 'adamuon', 'muon_nsr', 'muon_vs': 
          - 2D parameters (excluding embeddings and lm_head) use the specialized optimizer
          - Remaining parameters (embeddings, lm_head, 1D params) use AdamW
        
        Args:
            optimizer_name: Name of optimizer ('adam', 'muon', 'adamuon', 'muon_nsr', 'muon_vs')
            weight_decay: L2 regularization coefficient
            learning_rate: Base learning rate
            betas: Tuple of (beta1, beta2) for Adam-style optimizers
        ...
        # Separate 1D parameters (biases, layer norms) which should not have weight decay
        nodecay_params = [p for n, p in param_dict.items() if p.dim() < 2]
        
        if optimizer_name == 'adam':
            # Standard AdamW: all 2D+ params get weight decay
            decay_params = [p for n, p in param_dict.items() if p.dim() >= 2]
        else:
            # Muon-family optimizers: separate 2D params from embeddings/lm_head
            # twoD_params: 2D parameters excluding word embeddings and language model head
            twoD_params = [p for n, p in param_dict.items() if p.dim() >= 2 and ('wte' not in n and 'lm_head' not in n)]
            # decay_params: embeddings and lm_head (handled by AdamW with weight decay)
        ...
        if optimizer_name in ['muon', 'adamuon', 'muon_nsr', 'muon_vs']:
        ...
        # Configure AdamW optimizer for embeddings, lm_head, and 1D parameters
        ...
        # Configure Muon-family optimizer for 2D parameters
        if optimizer_name in ['muon', 'adamuon', 'muon_nsr', 'muon_vs']:
            # Get distributed training rank and world size
            try:
            ...
            if optimizer_name == 'muon':
                optimizer1 = Muon(twoD_params, lr=learning_rate, momentum=0.95, 
                                 rank=rank, world_size=world_size, weight_decay=weight_decay)
                print(f"using Muon and AdamW")
            elif optimizer_name == 'adamuon':
                optimizer1 = AdaMuon(twoD_params, lr=learning_rate, momentum=0.95, 
                                    rank=rank, world_size=world_size, weight_decay=weight_decay)
                print(f"using AdaMuon and AdamW")
            elif optimizer_name == 'muon_nsr':
                optimizer1 = Muon_NSR(twoD_params, lr=learning_rate, momentum=0.95, 
                                     rank=rank, world_size=world_size, weight_decay=weight_decay)
                print(f"using Muon_NSR and AdamW")
            elif optimizer_name == 'muon_vs':
                optimizer1 = Muon_VS(twoD_params, lr=learning_rate, momentum=0.95, 
                                    rank=rank, world_size=world_size, weight_decay=weight_decay)
                print(f"using Muon_VS and AdamW")
