"""
Muon-NSR Optimizer: Muon with Noise-to-Signal Ratio Preprocessing.

This optimizer extends the Muon optimizer by incorporating a noise-to-signal 
ratio (NSR) based preprocessing step before the Newton-Schulz orthogonalization.
The NSR preprocessing helps stabilize training by normalizing the momentum 
estimates based on their signal strength relative to variance.

Reference:
    - Muon: https://github.com/KellerJordan/modded-nanogpt
    - Moonlight LR scaling: https://github.com/MoonshotAI/Moonlight
"""
...
class Muon_NSR(torch.optim.Optimizer):
    """
    Muon optimizer with Noise-to-Signal Ratio (NSR) preprocessing.
    
    This optimizer combines momentum-based gradient updates with orthogonalization via
    Newton-Schulz iteration, enhanced with NSR-based normalization. The key innovation is
    preprocessing the momentum estimate with a noise-to-signal ratio term before 
    orthogonalization, which helps distinguish signal from noise in gradient estimates.
    ...
    Algorithm:
        ...
        5. NSR preprocessing: M_tilde = M_hat / sqrt(M_hat^2 + α*V_corrected + eps)
    ...
    Args:
        ...
        alpha: Scaling factor for variance in NSR computation. Default: 10.0
    ...
    Reference:
        - Muon: https://github.com/KellerJordan/modded-nanogpt
        - NSR preprocessing inspired by signal processing principles
    """
    ...
    def step(self):
        """
        Perform a single optimization step with NSR preprocessing.
        
        This method implements the full Muon-NSR algorithm:
        ...
        4. Apply NSR-based preprocessing (signal-to-noise normalization)
        ...
        for group in self.param_groups:
            ...
            for base_i in range(len(params))[::self.world_size]:
                if base_i + self.rank < len(params):
                    ...
                    # Step 5: NSR preprocessing (Noise-to-Signal Ratio normalization)
                    # M_tilde = M_hat / sqrt(M_hat^2 + α*V_corrected + eps)
                    # This normalizes the signal based on its strength relative to noise
